...
<!DOCTYPE xsl:stylesheet [
  ...
  <!ENTITY taxonomy	"http://ddbj.nig.ac.jp/ontologies/taxonomy/">
  <!ENTITY taxncbi     "http://www.ncbi.nlm.nih.gov/taxonomy/">
  <!ENTITY taxobo0     "http://purl.obolibrary.org/obo/NCBITaxon_">
  <!ENTITY taxobo1     "http://purl.org/obo/owl/NCBITaxon#">
  <!ENTITY taxobo2     "http://www.berkeleybop.org/ontologies/owl/NCBITaxon#">
  <!ENTITY taxup     "http://purl.uniprot.org/taxonomy/">
  <!ENTITY pubmed     "http://identifiers.org/pubmed/">
...
>
  ...
  <!-- global variables and functions -->
	...
	<xsl:variable name="nodeset-class"
		select=".//*[     
		   (rdf:type/@rdf:resource='&owl;Class'  or (local-name()='Class' and namespace-uri()='&owl;' and @rdf:about) or (owl:Class and @rdf:about) or (namespace-uri()='http://ddbj.nig.ac.jp/ontologies/taxonomy/' and @rdf:about))
  ]" />
...
<xsl:template match="rdf:RDF">
	...
	<xsl:template match="*" mode="details_ontology">
	    <h3>http://ddbj.nig.ac.jp/ontologies/taxonomy/<span style="padding-left: 30px"><u><a href="/ontologies/taxonomy.ttl">Turtle</a></u> <u><a href="/ontologies/taxonomy.rdf">RDF/XML</a></u></span></h3>
	...
	<xsl:template name="url">
		...
		<xsl:choose>
			...
			<xsl:when test="$ns='&taxonomy;'">
				<a href="{concat($ns,$name)}">
					:<xsl:value-of select="$name" />
				</a>
			</xsl:when>
			<xsl:when test="$ns='&taxncbi;'">
				<a href="{concat($ns,$name)}">
					taxncbi:<xsl:value-of select="$name" />
				</a>
			</xsl:when>
			<xsl:when test="$ns='&taxobo0;'">
				<a href="{concat($ns,$name)}">
					taxobo0:<xsl:value-of select="$name" />
				</a>
			</xsl:when>
			<xsl:when test="$ns='&taxobo1;'">
				<a href="{concat($ns,$name)}">
					taxobo1:<xsl:value-of select="$name" />
				</a>
			</xsl:when>
			<xsl:when test="$ns='&taxobo2;'">
				<a href="{concat($ns,$name)}">
					taxobo2:<xsl:value-of select="$name" />
				</a>
			</xsl:when>
			<xsl:when test="$ns='&taxup;'">
				<a href="{concat($ns,$name)}">
					taxup:<xsl:value-of select="$name" />
				</a>
			</xsl:when>
	...
	<xsl:template name="prettyUrl">
		...
		<xsl:choose>
			...
			<xsl:when test='starts-with($name, "&taxonomy;")'>
				<xsl:choose>				
					<xsl:when test='$name="&taxonomy;"'>
						<xsl:value-of select='$name' />	
					</xsl:when>
					<xsl:otherwise>
						:<xsl:value-of select='substring-after($name, "&taxonomy;")' />
					</xsl:otherwise>	
				</xsl:choose>
			</xsl:when>
			<xsl:when test='starts-with($name, "&taxncbi;")'>
				<xsl:choose>				
					<xsl:when test='$name="&taxncbi;"'>
						<xsl:value-of select='$name' />	
					</xsl:when>
					<xsl:otherwise>	
						taxncibi:<xsl:value-of select='substring-after($name, "&taxncbi;")' />
					</xsl:otherwise>	
				</xsl:choose>				
			</xsl:when>
			<xsl:when test='starts-with($name, "&taxobo0;")'>
				<xsl:choose>				
					<xsl:when test='$name="&taxobo0;"'>
						<xsl:value-of select='$name' />	
					</xsl:when>
					<xsl:otherwise>
						taxobo0:<xsl:value-of select='substring-after($name, "&taxobo0;")' />
					</xsl:otherwise>	
				</xsl:choose>
			</xsl:when>
			<xsl:when test='starts-with($name, "&taxobo1;")'>
				<xsl:choose>				
					<xsl:when test='$name="&taxobo1;"'>
						<xsl:value-of select='$name' />	
					</xsl:when>
					<xsl:otherwise>
						taxobo1:<xsl:value-of select='substring-after($name, "&taxobo1;")' />
					</xsl:otherwise>	
				</xsl:choose>
			</xsl:when>
			<xsl:when test='starts-with($name, "&taxobo2;")'>
				<xsl:choose>				
					<xsl:when test='$name="&taxobo2;"'>
						<xsl:value-of select='$name' />	
					</xsl:when>
					<xsl:otherwise>
						taxobo2:<xsl:value-of select='substring-after($name, "&taxobo2;")' />
					</xsl:otherwise>	
				</xsl:choose>
			</xsl:when>
			<xsl:when test='starts-with($name, "&taxup;")'>
				<xsl:choose>				
					<xsl:when test='$name="&taxup;"'>
						<xsl:value-of select='$name' />	
					</xsl:when>
					<xsl:otherwise>
						taxup:<xsl:value-of select='substring-after($name, "&taxup;")' />
					</xsl:otherwise>	
				</xsl:choose>
			</xsl:when>
